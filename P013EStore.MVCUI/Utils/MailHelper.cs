...
namespace P013EStore.MVCUI.Utils
{
    public class MailHelper
    {
        public static async Task SendMailAsync(Contact contact, string konu = "Siteden mesaj geldi")
        {
            ...
            smtpClient.Credentials = new NetworkCredential("email kullanıcı adı", "email şifre"); // email sunucusu ssl ile çalışıyorsa true ver
            ...
            message.IsBodyHtml =true; // Gönderimde html kodu kullandıysak bu ayarı aktif demeliyiz.
            // smtpClient.Send(message); // mesajı asenkron olarak gönderir.
            await smtpClient.SendMailAsync(message);//mesajı asenkron olarak mail attık.
            smtpClient.Dispose(); // nesneyi bellekten at demek.
